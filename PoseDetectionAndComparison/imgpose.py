...
for i in range(17):
    for n in range(len(personwiseKeypoints)):
        ...
        B = np.int32(keypoints_list[index.astype(int), 0])
        A = np.int32(keypoints_list[index.astype(int), 1])
        cv2.line(frameClone_model, (B[0], A[0]), (B[1], A[1]), colors[i], 3, cv2.LINE_AA)

modelVector = getVectors(personwiseKeypoints, detected_keypoints)[0]

image1 = cv2.imread("same.jpg")
net = cv2.dnn.readNetFromCaffe(protoFile, weightsFile)
...
for pvec in currVector:
    ...
    if similarity > 0.9:
        ...
        cv2.putText(frameClone, "Match - Acc:" + str(int(accuracy))+"%", (pt1[0], pt1[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 100), 2)
image_1= np.concatenate((frameClone_model, frameClone), axis=1)
